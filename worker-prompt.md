...
For each folder, in order:
   ...
   f. **Write or update `results.html`** — the actual deliverable the owner opens in a file previewer (e.g. Finder Quick Look). Use clean inline styling (no external assets). On re-engagement, add a clearly-labeled new section for this round **at the TOP of the document (newest-first)** — insert the new round above all prior rounds so the latest work is the first thing the owner sees, with earlier rounds following below in descending order. Never lose prior content; if restructuring reads better, keep the newest-first ordering. (For a brand-new project's first write, ordering is moot — just write the deliverable; this matters once a second round exists.)

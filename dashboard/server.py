...
def compute_health(folder: Path, status, has_instructions: bool,
                   last_processed, is_archived: bool) -> dict:
    ...
    if is_archived:
        ...
        if stale_days < HEALTH_MOVE_DAYS:
            light = "active"
        elif stale_days < HEALTH_SLIP_DAYS:
            light = "move"
        elif stale_days < HEALTH_STALLED_DAYS:
            light = "slipping"
        else:
            light = "stalled"
...
def project_record(folder: Path, archived: bool = False, future: bool = False) -> dict:
    name = folder.name
    base = name
...
def all_projects() -> list[dict]:
    out = [project_record(p, archived=False) for p in list_project_dirs()]
    out.extend(project_record(p, archived=True) for p in list_archived_dirs())
    out.extend(project_record(p, future=True) for p in list_future_dirs())
    return out
